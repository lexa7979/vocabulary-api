...
function runTestsAboutCopyObject() {
  describe(`has a function ${bold('copyObject()')} which`, () => {
    ...
    it(`- when used w/o arguments - ${RETURNS} undefined`, () =>
      expect(copyObject()).toBeUndefined());
    ...
    it(`- when "input" includes a circular reference - ${green(
      'replaces'
    )} the reference with a string`, () => {
      const obj = {
        a: {},
      };
      obj.a.ref = obj;
    ...
    it(`- when used with option "replaceFunctions" - ${green(
      'replaces'
    )} all functions with strings`, () => {
      // @ts-ignore
...
function runTestsAboutRegisterReplaceAction() {
  describe(`has a function ${bold('registerReplaceAction()')} which`, () => {
    ...
    it(`- when used with "optionsName" and "replaceCallback" - ${green(
      'memorizes'
    )} new action`, () => {
      Copy._testInternals.resetReplaceActions();
    ...
    it(`- when used a second time with same arguments - ${red(
      'IGNORES'
    )} new action and ${RETURNS} false`, () => {
      Copy._testInternals.resetReplaceActions();
