...
describe('Test-data service "words"', () => {
    it(`has the expected exports`, () => {
        expect(Words).toEqual({
            getTranslation: expect.any(Function),
            getWord: expect.any(Function),
            getWordWithTranslations: expect.any(Function),
            listAllTranslationsOfWord: expect.any(Function),
            listAllWords: expect.any(Function),
        });
    });
...
function runTestsAboutGetWord() {
    describe(`${_nextListHint()} has an ${ASYNC} function ${bold('getWord()')} which`, () => {
        ...
        it(`- when used w/o arguments - ${REJECTS} as expected`, () => expect(getWord).rejects.toThrow('invalid ID'));
...
function runTestsAboutGetWordWithTranslations() {
    describe(`${_nextListHint()} has an ${ASYNC} function ${bold('getWordWithTranslations')} which`, () => {
...
function runTestsAboutListAllWords() {
    describe(`${_nextListHint()} has an ${ASYNC} function ${bold('listAllWords()')} which`, () => {
        ...
        it(`- when used with argument "first" - ${RESOLVES} as expected`, async () => {
            ...
            const copy = copyObject(result, { replaceUUIDs: true });
            expect(copy).toEqual([
                { classId: '(ID:verb)', id: '(ID:word1)' },
                { classId: '(ID:noun)', id: '(ID:word2)' },
                { classId: '(ID:adjective)', id: '(ID:word3)' },
                { classId: '(ID:noun)', id: '(ID:word4)' },
            ]);
...
function runTestsAboutListAllTranslationsOfWord() {
    describe(`${_nextListHint()} has an ${ASYNC} function ${bold('listAllTranslationsOfWord()')} which`, () => {
