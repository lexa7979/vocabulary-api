...
class NextLearnStepsFinder {
  /**
   * @param {{ db: object, userId: string, nextStepsCount: number }} inputBag
   */
  ...
  async process() {
    ...
    const _calculateRankingAndAddToAvailableLearnSteps = activeWord =>
      this._processOneActiveWord({ activeWord });
  ...
  _countPreparedAndNeededSteps() {
    if (
      this.userLearnPath.some(pathItem => typeof pathItem.step !== 'number' || pathItem.step <= 0)
    ) {
    ...
    this.numberOfNeededNewSteps = Math.max(
      0,
      this.userCurrStep + this.nextStepsCount - this.userLastPreparedStep
    );
  }
  ...
  async _processOneActiveWord({ activeWord }) {
    ...
    if (typeof wordId !== 'string' || wordId === '') {
      throw new Error(
        `got invalid property "wordId" with active-word ${id} of user ${this.userId}`
      );
    }
    if (!Array.isArray(learnProgress) || learnProgress.some(_isNoObject)) {
      throw new Error(
        `got invalid property "learnProgress" with active-word ${id} of user ${this.userId}`
      );
    }
    ...
    const activeWordIsAlreadyPrepared =
      latestProgressStep != null && latestProgressStep >= this.userCurrStep;
    ...
    const activeWordAlreadyUsedShortly =
      timeOfLastProgress + MINIMUM_TIME_BEFORE_REPEATING_WORD > this.now;
    ...
    for (let i = 1; i <= this.numberOfNeededNewSteps; i += 1) {
      ...
      for (
        ...
        const available = this.availableLearnSteps
          ...
          .filter(
            item => item.latestProgressStep == null || step >= item.latestProgressStep + stepDiff
          );
 ...
 * @param {object} inputBag.db
 * @param {string} inputBag.userId
